...
(For an alternative to modifying the `/etc/hosts` file, see [Assigning an IP Address for remote method invocation](../../10_appendix/03_miscellaneous_xdb_processing_topics/01_publications_and_subscriptions_server_conf_options/07_assign_ip_adress_for_rmi/#assign_ip_adress_for_rmi).)
