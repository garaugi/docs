...
BigAnimal supports the following cluster types: 
- Single node

- Standard high availability

<FeatureFlag flagName="prod-in-preview"> 
...
EDB Postgres Distributed uses bi-directional logical replication to keep data nodes synchronized. By default, BigAnimal uses the [Group Commit](/pgd/latest/durability/group-commit/) functionality of this replication to synchronously ensure that transactions are received by more than one node in a group. This works alongside [Lag Control](/pgd/latest/durability/lag-control/), an asynchronous mechanism that manages commit behavior. Both Group Commit and Lag Control are configured using [Commit Scopes](/pgd/latest/durability/commit-scopes). To learn more about configuring Commit Scopes, see [Configuration](/pgd/latest/durability/commit-scopes/#configuration).

While Group Commit is the default commit behavior for data nodes, it is possible to configure it to be used on a per transaction basis. 

With Group Commit enabled, [Transaction streaming](/pgd/latest/transaction-streaming/) is disabled.

!!! Note 
    There is a default value set for [`max_prepared_transactions`](/pgd/latest/configuration/#max-prepared-transactions) on Group Commit. This is the setting for the maximum number of concurrent transactions across the cluster. Check that it is set high enough to handle the number of transactions originating per node in your cluster. Check that the `max_prepared_transactions` value is high enough to handle the number of transactions originating per node in you cluster. 
