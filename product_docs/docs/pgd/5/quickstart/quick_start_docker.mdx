...
We specify the PGD-Always-ON architecture (`--architecture PGD-Always-ON`): this
sets up the configuration for [PGD 5's Always On
architectures](/pgd/latest/architectures/). As part of the default architecture,
it configures our cluster with three data nodes, co-hosting three [PGD
Proxy](routing/proxy/) servers, along with a [Barman](backup#physical-backup)
node for backup.

We specify using Docker (`--platform docker`). By default, TPA configures Rocky
